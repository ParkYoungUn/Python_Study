...
class Marine(AttackUnit):
    ...
    # 스팀팩 : 일정 시간동안 공격력, 공격속도, 이동속도 증가, 체력 10 소모
    def stimpack(self):
        if self.hp > 10:
            self.hp -= 10
            print("{} : 스팀팩을 사용합니다. (HP 10 감소)".format(self.name))
        else:
            print("{} : 체력이 부족하여 스팀팩을 사용하지 못합니다.".format(self.name))


class Tank(AttackUnit):
    # 시즈모드 : 탱크를 지상에 고정 시켜, 더 높은 파워로 공격 가능, 이동불가
...
class wraith(FlyableAttackUnit):
    ...
    def clocking(self):
        ...
        # 클로킹 모드가 아닐 때
            ...
            print("{} : 클로킹 모드를 설정 합니다.".format(self.name))
            self.clocked = True
...
for unit in attack_units:
    # isinstance() 지금 만들어진 객체가 어떤 클래스의 인스턴스 인지 확인.
...
# 전군 피해
for unit in attack_units:
    unit.damaged(randint(5, 21))  # 공격을 받음 (5~20)
